...
inputID.val(response.variation.id);inputID.trigger('change')}}})}}};AddToCartHandler.prototype.onAddToCart=function(e){$(product_detail_alert).html('');let button=$(this);let btnTxt=button.html();button.html('<i class="fas fa-circle-notch fa-spin"></i>');let data=$(':input',$(product_detail_form)).serializeJSON();data.product_id=$(this).attr('data-id');data.qty=$('#quantity').val();if(typeof data.qty=='undefined')data.qty=1;data.action='Cart_Ajax::addCart';$.post(ajax,data,function(){},'json').done(function(response){button.html(btnTxt);show_message(response.message,response.status);if(response.status==='success'){$('.wcmc-total-items').html(response.total_items);$(element_cart_content+' '+element_cart_tbody).html(response.items);$('.cart-total-price').html(response.total_label);let cart_sidebar=$('#cart-sidebar');if(typeof cart_sidebar.html()!=='undefined'){$('.panel--sidebar').removeClass('active');$('body').removeClass('panel__sidebar-opened');cart_sidebar.addClass('active')}}});return!1};AddToCartHandler.prototype.onAddToCartNow=function(e){$(product_detail_alert).html('');let button=$(this);let btnTxt=button.html();button.html('<i class="fas fa-circle-notch fa-spin"></i>');let data=$(':input',$(product_detail_form)).serializeJSON();data.product_id=$(this).attr('data-id');data.qty=$('#quantity').val();if(typeof data.qty=='undefined')data.qty=1;data.action='Cart_Ajax::addCart';$.post(ajax,data,function(){},'json').done(function(response){button.html(btnTxt);show_message(response.message,response.status);if(response.status==='success'){$('.wcmc-total-items').html(response.total_items);window.location='gio-hang'}});return!1};var addtocart=new AddToCartHandler();let cartInput,cartNumber,cartQty,cartQtyOld,cartQtyClickTimer,cartQtyDoneClickInterval=500;let CartHandler=function(){$(document).on('click','.plus',this.plusQty).on('click','.minus',this.minusQty).on('click','.plus, .minus',this.timeClick).on('click','.cart_item__trash, .js_cart_item__trash_close',this.showButtonDelete).on('click','.js_cart_item__trash_agree',this.deleteItem)};CartHandler.prototype.plusQty=function(e){let box=$(this).closest('.quantity');cartInput=box.find('input.qty');cartNumber=box.find('div.qty');cartQty=parseInt(cartInput.val());cartQtyOld=cartQty;cartQty+=1;cartInput.val(cartQty);cartNumber.text(cartQty);return!1};CartHandler.prototype.minusQty=function(e){let box=$(this).closest('.quantity');cartInput=box.find('input.qty');cartNumber=box.find('div.qty');cartQty=parseInt(cartInput.val());cartQtyOld=cartQty;if(cartQty>1){cartQty-=1;cartInput.val(cartQty);cartNumber.text(cartQty)}
return!1};CartHandler.prototype.timeClick=function(e){clearTimeout(cartQtyClickTimer);cartQtyClickTimer=setTimeout(CartHandler.prototype.qtyDoneClick($(this)),cartQtyDoneClickInterval)};CartHandler.prototype.qtyDoneClick=function(e){$(element_cart_content+' .loading').show();let box=e.closest(element_cart_item);let data={'action':'Cart_Ajax::updateQuantity','rowid':box.find('input[name="rowid"]').val(),'qty':box.find('input.qty').val(),};$.post(base+'/ajax',data,function(){},'json').done(function(response){$(element_cart_content+' .loading').hide();console.log(response);if(response.status==='success'){box.find('.js_cart_item_price').html((response.price).toLocaleString());$('#cart-total-price, .cart-total-price').html(response.total);$('#summary-cart-total-price, .summary-cart-total-price').html(response.summary_total)}else{$('.cart-error').html(response.message.error);cartInput.val(cartQtyOld);cartNumber.text(cartQtyOld)}})};CartHandler.prototype.showButtonDelete=function(e){$(this).closest(element_cart_item).find('.cart_item__trash_popover').toggleClass('active')};CartHandler.prototype.deleteItem=function(e){let box=$(this).closest(element_cart_item);let data={'action':'Cart_Ajax::updateQuantity','rowid':box.find('input[name="rowid"]').val(),'qty':0};$.post(base+'/ajax',data,function(){},'json').done(function(response){if(response.status==='success'){box.remove();if(response.total===0){location.reload()}else{$('#cart-total-price').html(response.total);$('#summary-cart-total-price').html(response.summary_total)}}})};var cart=new CartHandler();const element_checkout=(typeof $('.woocommerce-checkout').html()!=='undefined')?'.woocommerce-checkout':'.page-checkout';const element_review=(typeof $('.wcm-box-order').html()!=='undefined')?$('.wcm-box-order'):$('.page-checkout-review');const element_checkout_shipping=$('input[name="show-form-shipping"]');let CheckoutHandler=function(){$(document).on('submit',element_checkout,this.submitCheckout).on('change','#billing_city',this.loadDistricts).on('change','#billing_districts',this.loadWard).on('change','#billing_ward',this.checkoutReview).on('change','#shipping_city',this.loadShippingDistricts).on('change','#shipping_districts',this.loadShippingWard).on('change','#shipping_ward',this.checkoutReview).on('change',element_checkout+' input[name="shipping_type"]',this.checkoutReview).on('change',element_checkout+' input[name="show-form-shipping"]',this.checkoutReview)};CheckoutHandler.prototype.submitCheckout=function(e){let data=$(':input',$(this)).serializeJSON();data.action='Cart_Ajax::saveCheckout';element_review.addClass('scmc-loading');$.post(ajax,data,function(){},'json').done(function(data){element_review.removeClass('scmc-loading');$(element_checkout+' .notice').remove();$(element_checkout+' .toast').remove();$('.error_message').html('');$('.input_checkout').removeClass('error_show').removeClass('error_input');if(data.status==='success'){window.location=data.url}else{let count_error=0;let notice=data.message;for(const[index,message]of Object.entries(notice)){if(typeof $('#error_'+index).html()!='undefined'){count_error++;$('#error_'+index).closest('.input_checkout').addClass('error_input');$('#error_'+index).html(message)}else{$(element_checkout).prepend(message)}}
if(count_error>0){$('.input_checkout').addClass('error_show')}
if(typeof $(element_checkout+' .toast').html()!='undefined'){$('html, body').animate({scrollTop:$(element_checkout+' .toast').offset().top-100},500)}else{if(typeof $(element_checkout+' .error_input').html()!='undefined'){$('html, body').animate({scrollTop:$(element_checkout+' .error_input').offset().top-100},500)}}}});return!1};CheckoutHandler.prototype.loadDistricts=function(e){let data={province_id:$(this).val(),action:'Cart_Ajax::loadDistricts'};$jqxhr=$.post(ajax,data,function(){},'json');$jqxhr.done(function(response){if(response.status==='success'){$('#billing_districts').html(response.data);$('#billing_ward').html('<option value="">Chọn phường xã</option>');if(element_checkout_shipping.prop('checked')===!1){CheckoutHandler.prototype.checkoutReview()}}});return!1};CheckoutHandler.prototype.loadWard=function(e){let data={district_id:$(this).val(),action:'Cart_Ajax::loadWard'};$.post(ajax,data,function(){},'json').done(function(response){if(response.status==='success'){$('#billing_ward').html(response.data);if(element_checkout_shipping.prop('checked')===!1){CheckoutHandler.prototype.checkoutReview()}}});return!1};CheckoutHandler.prototype.loadShippingDistricts=function(e){let data={province_id:$(this).val(),action:'Cart_Ajax::loadDistricts'};$.post(ajax,data,function(){},'json').done(function(response){if(response.status==='success'){$('#shipping_districts').html(response.data);$('#shipping_ward').html('<option value="">Chọn phường xã</option>');if(element_checkout_shipping.prop('checked')===!0){CheckoutHandler.prototype.checkoutReview()}}});return!1};CheckoutHandler.prototype.loadShippingWard=function(e){let data={district_id:$(this).val(),action:'Cart_Ajax::loadWard'};$.post(ajax,data,function(){},'json').done(function(response){if(response.status==='success'){$('#shipping_ward').html(response.data);if(element_checkout_shipping.prop('checked')===!0){CheckoutHandler.prototype.checkoutReview()}}});return!1};CheckoutHandler.prototype.checkoutReview=function(e){element_review.addClass('scmc-loading');let data=$('form[name="checkout"]').serializeJSON();data.action='Cart_Ajax::loadCheckoutReview';$.post(ajax,data,function(){},'json').done(function(response){if(response.type==='success'){element_review.html(response.order_review);element_review.removeClass('scmc-loading')}})};var checkout=new CheckoutHandler();if(typeof element_review.html()!='undefined'){checkout.checkoutReview()}});function update_order_review(){let element_review=(typeof $('.wcm-box-order').html()!=='undefined')?$('.wcm-box-order'):$('.page-checkout-review');element_review.addClass('scmc-loading');let data=$('form[name="checkout"]').serializeJSON();data.action='Cart_Ajax::loadCheckoutReview';$.post(ajax,data,function(){},'json').done(function(response){if(response.type==='success'){element_review.html(response.order_review);element_review.removeClass('scmc-loading')}})}
...
return propCount}
$(function(){$(function(){var hasCount=[];function counterRun(id){let oTop=$('#js_counter_section_'+id+' .counter-item').offset().top-window.innerHeight;if(typeof hasCount[id]=='undefined'&&$(window).scrollTop()>oTop){$('#js_counter_section_'+id+' .counter-item .counter').each(function(){let $this=$(this),countTo=$this.attr("data-number");$({countNum:$this.text()}).animate({countNum:countTo},{duration:850,easing:"swing",step:function(){$this.text(Math.ceil(this.countNum).toLocaleString("en"))},complete:function(){$this.text(Math.ceil(this.countNum).toLocaleString("en"))}})});hasCount[id]=id}}
$('.counter-section-container').each(function(){counterRun($(this).data('id'))});$(window).scroll(function(){$('.counter-section-container').each(function(){counterRun($(this).data('id'))})});$('.widget_about_style_9 .item-list .item-heading').click(function(){$(this).closest('.item-list').find('.item').removeClass('active');$(this).closest('.item').addClass('active');return!1})});$(function(){let widgetLoad=[],categoryId,widgetId,data_item=[];if('IntersectionObserver' in window){let productListWidget=document.querySelectorAll('.js_product_style_1_data');let observer=new IntersectionObserver((entries)=>{entries.forEach(entry=>{if(entry.isIntersecting){let id=entry.target.getAttribute('data-id');if(typeof widgetLoad[id]=='undefined'){widgetLoad[id]=id;product_style_1_slider_load(entry.target.getAttribute('data-category'),entry.target.getAttribute('data-id'),JSON.parse(entry.target.getAttribute('data-options')))}}})});productListWidget.forEach(widget=>{observer.observe(widget)})}else{$('.widget_product_style_1 .js_product_style_1_data').each(function(){let options=$(this).data('options');product_style_1_slider_load($(this).data('category'),$(this).data('id'),options)})}
function product_style_1_slider(id,options){if(options.display.type===1)return!1;let productList='#product_style_1_content_'+id+' .swiper';let productBtnNext=$('#product_style_1_content_'+id+' .next');let productBtnPrev=$('#product_style_1_content_'+id+' .prev');function shouldBeEnabled(carousel,numberShow){const slidesCount=carousel.find('.swiper-slide').length;if(slidesCount<numberShow){return{loop:!1,}}
return{loop:!0,}}
...
function product_style_1_slider_load(categoryId,widgetId,options){let productBox=$('.js_widget_product_style_1_'+widgetId);let productList=productBox.find('.list-product');let productLink=productBox.find('a.more-link');let loading=productBox.find('.wg-loading');options.display.type=parseInt(options.display.type);productList.html('');loading.show();if(typeof data_item[widgetId]=='undefined')data_item[widgetId]=[];if(typeof data_item[widgetId][categoryId]!='undefined'){productList.html(data_item[widgetId][categoryId].item);productLink.attr('href',data_item[widgetId][categoryId].url);loading.hide();product_style_1_slider(widgetId,options);return!1}else{let data={action:'widget_product_style_1::loadProduct',widgetId:widgetId,categoryId:categoryId,};$.post(ajax,data,function(){},'json').done(function(response){loading.hide();if(response.status==='success'){productList.html(response.item);productLink.attr('href',response.slug);data_item[widgetId][categoryId]={item:response.item,url:response.slug};product_style_1_slider(widgetId,options)}})}}});$(function(){let widgetLoad=[],tabId,widgetId,data_item=[];if('IntersectionObserver' in window){let productListWidget=document.querySelectorAll('.js_product_style_8_data');let observer=new IntersectionObserver((entries)=>{entries.forEach(entry=>{if(entry.isIntersecting){let id=entry.target.getAttribute('data-id');if(typeof widgetLoad[id]=='undefined'){widgetLoad[id]=id;product_style_8_slider_load(entry.target.getAttribute('data-tab'),entry.target.getAttribute('data-id'),JSON.parse(entry.target.getAttribute('data-options')))}}})});productListWidget.forEach(widget=>{observer.observe(widget)})}else{$('.widget_product_style_8 .js_product_style_8_data').each(function(){let options=$(this).data('options');product_style_8_slider_load($(this).data('tab'),$(this).data('id'),options)})}
$(document).on('click touch','.product_style_8_header .product_style_8_category_list li.item a',function(){tabId=$(this).attr('data-tab');widgetId=$(this).closest('.js_product_style_8_data').attr('data-id');$('#product_style_8_header_'+widgetId+' .product_style_8_category_list li.item a').removeClass('active');$(this).closest('.js_product_style_8_data').attr('data-tab',tabId);$(this).addClass('active');product_style_8_slider_load(tabId,widgetId,$(this).closest('.product_style_8_header').data('options'));return!1});function product_style_8_slider(id,options){if(options.display.type===1)return!1;let productList='#product_style_8_content_'+id+' .swiper';let productBtnNext=$('#product_style_8_content_'+id+' .next');let productBtnPrev=$('#product_style_8_content_'+id+' .prev');function shouldBeEnabled(carousel,numberShow){const slidesCount=carousel.find('.swiper-slide').length;if(slidesCount<numberShow){return{loop:!1,}}
return{loop:!0,}}
let config={...shouldBeEnabled($(productList),parseInt(options.numberShow)),autoplay:{delay:parseInt(options.display.time)*1000},speed:500,slidesPerView:parseInt(options.numberShow),spaceBetween:parseInt(getComputedStyle(document.body).getPropertyValue('--bs-gutter-x')),breakpoints:{0:{...shouldBeEnabled($(productList),parseInt(options.numberShowMobile)),slidesPerView:parseInt(options.numberShowMobile)},768:{...shouldBeEnabled($(productList),parseInt(options.numberShowTablet)),slidesPerView:parseInt(options.numberShowTablet)},1200:{...shouldBeEnabled($(productList),parseInt(options.numberShow)),slidesPerView:parseInt(options.numberShow)},},}
let swiper=new Swiper(productList,config);productBtnNext.click(function(){swiper.slideNext()});productBtnPrev.click(function(){swiper.slidePrev()})}
function product_style_8_slider_load(tabId,widgetId,options){let productBox=$('.js_widget_product_style_8_'+widgetId);let productList=productBox.find('.list-product');let productLink=productBox.find('a.more-link');let loading=productBox.find('.wg-loading');options.display.type=parseInt(options.display.type);productList.html('');loading.show();if(typeof data_item[widgetId]=='undefined')data_item[widgetId]=[];if(typeof data_item[widgetId][tabId]!='undefined'){productList.html(data_item[widgetId][tabId].item);productLink.attr('href',data_item[widgetId][tabId].url);loading.hide();product_style_8_slider(widgetId,options);return!1}else{let data={action:'widget_product_style_8::loadProduct',widgetId:widgetId,tabId:tabId,};$.post(ajax,data,function(){},'json').done(function(response){loading.hide();if(response.status==='success'){productList.html(response.item);productLink.attr('href',response.slug);data_item[widgetId][tabId]={item:response.item,url:response.slug};product_style_8_slider(widgetId,options)}})}}})})
$(function(){$(document).on('click','.btn-quickview',function(e){e.preventDefault();let pid=$(this).attr('data-id');$.ajaxSetup({beforeSend:function(xhr,settings){settings.data+='&csrf_test_name='+encodeURIComponent(getCookie('csrf_cookie_name'))}});$.fancybox.open({src:base+'/ajax?action=quickview_ajax_product_load&id='+pid,type:'ajax',opts:{onComplete:function(){console.log(pid)}}})});$(document).on('click','.fancybox-close-small',function(){$.fancybox.close()})})