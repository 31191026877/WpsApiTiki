...
class MY_Profiler extends CI_Profiler {
    ...
    protected function _compile_queries() {
        ...
        if(have_posts($databaseQuery)) {
            ...
            $dbs['db'] = (object)['queries' => [], 'query_times' => [], 'database' => CLE_DBNAME];

            foreach ($databaseQuery as $item) {
                $dbs['db']->queries[] = $item['query'] .' -- '.implode(',', $item['bindings']);
                $dbs['db']->query_times[] = $item['time'];
            }
        }
