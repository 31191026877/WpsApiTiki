...
<head>
    ...
    <link href="css/style.css?v=1.<?php echo $version; ?>" rel="stylesheet" type="text/css"/>
    <!--[if lt IE 8]>
    <style>
    ...
    <script src="js/jquery.jplayer-2.9.2.min.js"></script>
    <script src='js/fabric-3.6.0.js'></script>
...
<script src="js/load-image.all.min.js?v=5.6.0"></script>
<!-- The Canvas to Blob plugin is included for image resizing functionality -->
<script src="//blueimp.github.io/JavaScript-Canvas-to-Blob/js/canvas-to-blob.min.js"></script>
<!-- The Iframe Transport is required for browsers without support for XHR file uploads -->
<script src="js/jquery.iframe-transport.js?v=10.0.2"></script>
<!-- The basic File Upload plugin -->
<script src="js/jquery.fileupload.js?v=10.0.2"></script>
<!-- The File Upload processing plugin -->
<script src="js/jquery.fileupload-process.js?v=10.0.2"></script>
<!-- The File Upload image preview & resize plugin -->
<script src="js/jquery.fileupload-image.js?v=10.0.2"></script>
<!-- The File Upload audio preview plugin -->
<script src="js/jquery.fileupload-audio.js?v=10.0.2"></script>
<!-- The File Upload video preview plugin -->
<script src="js/jquery.fileupload-video.js?v=10.0.2"></script>
<!-- The File Upload validation plugin -->
<script src="js/jquery.fileupload-validate.js?v=10.0.2"></script>
<!-- The File Upload user interface plugin -->
<script src="js/jquery.fileupload-ui.js?v=10.0.2"></script>
...
<input type="hidden" id="field_id" value="<?php echo (!empty($field_id)) ? str_replace(['[', ']'], ['\[', '\]'], $field_id) : ''; ?>"/>
...
<div class="container-fluid">
    ...
    foreach ($files as $file_array) {
        ...
        if (
            ...
            or (
                ...
                and $file_array[ 'extension' ] == fix_strtolower(trans('Type_dir'))
            )
            or (
    ...
    if ($config['load_more']) {
        ...
        $load_more_start = (isset($_GET['load_more_start']) and is_numeric($_GET['load_more_start'])) ? intval($_GET['load_more_start']) : 0;
            ...
            <ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
                ...
                foreach ($files as $file_array) {
                    ...
                    if ($file == '.'
                        ...
                        || ($file == '..' && $subdir == '') || in_array( $file, $config['hidden_folders'] )
                    ...
                    ?>
                    <li data-name="<?php echo $file ?>" class="<?php if ($file == '..') { echo 'back'; } else { echo 'dir'; } ?><?php if (!$config['multiple_selection']) { ?>no-selector<?php } ?>" <?php if (($filter != '' && stripos($file, $filter) === false)) { echo ' style="display:none;"'; } ?>>
                        <?php
                        ...
                        if ($file == "..") { echo "back-"; } ?>directory" data-type="<?php if ($file != "..") { echo "dir"; } ?>">
                            <?php
                            if ($file == "..") { ?>
                        ...
                        <?php
                            ...
                            <figcaption>
                                <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($config['rename_folders'] && !$file_prevent_rename) { echo "rename-folder"; } ?>" title="<?php echo trans('Rename') ?>" data-folder="1" data-permissions="<?php echo $file_array['permissions']; ?>"> <i class="icon-pencil <?php if (!$config['rename_folders'] || $file_prevent_rename) { echo 'icon-white'; } ?>"></i></a>
                                <a href="javascript:void('')" class="tip-left erase-button <?php if ($config['delete_folders'] && !$file_prevent_delete) { echo "delete-folder"; } ?>" title="<?php echo trans('Erase') ?>" data-confirm="<?php echo trans('Confirm_Folder_del'); ?>"><i class="icon-trash <?php if (!$config['delete_folders'] || $file_prevent_delete) { echo 'icon-white'; } ?>"></i></a>
                            </figcaption>
                        <?php
                        } ?>
                        </figure>
                ...
                ?>
                <li class="ff-item-type-<?php echo $class_ext; ?> file <?php if (!$config['multiple_selection']) { ?>no-selector<?php } ?>" data-name="<?php echo $file; ?>" <?php if (($filter != '' && stripos($file, $filter) === false)) { echo ' style="display:none;"';  } ?>>
                    <?php
                    $file_prevent_rename = false;
                    $file_prevent_delete = false;
                    if (isset($filePermissions[$file])) {
                        ...
                        <a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-function="<?php echo $apply; ?>">
                            <div class="img-precontainer">
                                <?php if ($is_icon_thumb) { ?>
                                    <div class="filetype"><?php echo $file_array['extension'] ?></div>
                                <?php } ?>
                            ...
                            <div class="img-precontainer-mini <?php if ($is_img) echo 'original-thumb' ?>">
                                <?php if ($config['multiple_selection']) { ?><?php } ?>
                            ...
                            <?php } ?>
                            <div class="box">
                                <h4 class="<?php if ($config['ellipsis_title_after_first_row']) { echo "ellipsis"; } ?>"> <?php echo $filename; ?></h4>
                            </div>
                        </a>
                        <input type="hidden" class="date" value="<?php echo $file_array['date']; ?>"/>
                        <input type="hidden" class="size" value="<?php echo $file_array['size'] ?>"/>
                        ...
                        <figcaption>
                            <form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
                                ...
                                <?php } elseif (($is_video || $is_audio) && in_array( $file_array['extension'], $config['jplayer_exts'] )) { ?>
                                    <a class="tip-right modalAV <?php if ($is_audio) { echo "audio"; } else { echo "video"; } ?>" title="<?php echo trans('Preview') ?>" data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>" href="javascript:void('');"><i class=" icon-eye-open"></i></a>
                                ...
                                <?php } ?>
                                <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($config['rename_files'] && !$file_prevent_rename) { echo "rename-file"; } ?>" title="<?php echo trans('Rename') ?>" data-folder="0" data-permissions="<?php echo $file_array['permissions']; ?>"> <i class="icon-pencil <?php if (!$config['rename_files'] || $file_prevent_rename) { echo 'icon-white'; } ?>"></i></a>
                                <a href="javascript:void('')" class="tip-left erase-button <?php if ($config['delete_files'] && !$file_prevent_delete) { echo "delete-file"; } ?>" title="<?php echo trans('Erase') ?>" data-confirm="<?php echo trans('Confirm_del'); ?>"> <i class="icon-trash <?php if (!$config['delete_files'] || $file_prevent_delete) { echo 'icon-white'; } ?>"></i></a>
                            </form>
                        </figcaption>
                    </figure>
                </li>
    ...
    <div id="load-more-container" data-start="<?php echo htmlspecialchars($config['load_more_limit']); ?>" data-auto="<?php echo ($config['load_more_auto'] ? '1' : '0'); ?>">
        ...
        <?php if (!$config['load_more_auto']): ?>
            <div style="padding:2em 0; text-align:center;">
                <button id="btn-load-more" type="button" class="btn" style="background-color: red; color:#fff; border-radius: 5px;">Tải Thêm</button>
            </div>
        <?php endif; ?>
    </div>
<?php endif; ?>
